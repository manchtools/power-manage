...
- **[ART-1]** ONE content-addressed blob store: key = SHA-256 of the content,
  ...
  - The size cap is enforced at upload against the binary's typed config
    (SPEC-002). Default: 512 MiB; hard maximum: 900 MiB. The hard maximum is
    dictated by the backing: a Postgres `bytea` field caps at 1 GiB − 1, and
    the margin keeps encode/decode memory bounded. Rationale for the default:
    the largest legitimate blob class is a vendor `PACKAGE_FILE` payload
    (multi-hundred-MB `.deb`/`.rpm` bundles); upload and fetch are chunked, so
    the cap protects Postgres row and backup growth, not memory. Operators
    shipping larger vendor bundles raise the knob explicitly up to the hard
    maximum (INV-18 knob rationale, SPEC-002); beyond it, the object-storage
    seam — not a bigger `bytea` — is the answer.
