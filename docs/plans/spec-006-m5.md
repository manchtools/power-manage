...
```bash
set -e -o pipefail
go test -C contract ./... -count=1 -race 2>&1 | tee /tmp/spec-006-m5-contract.log
go test -C server ./... -count=1 -race 2>&1 | tee /tmp/spec-006-m5-server.log
go test -C agent ./... -count=1 -race 2>&1 | tee /tmp/spec-006-m5-agent.log
go test -C sdk ./... -count=1 -race 2>&1 | tee /tmp/spec-006-m5-sdk.log
./scripts/verify.sh 2>&1 | tee /tmp/spec-006-m5-verify.log
docref check 2>&1 | tee /tmp/spec-006-m5-docref.log
```

- Focused tests precede the four module suites above.
- Run a local full-diff review before publication.
