...
ATTR_PATTERNS=(
  ...
  # Deliberately broad: any Capitalized tool after "generated with/by", so a
  # new AI vendor cannot slip through a name allowlist. Ceiling: a body like
  # "Regenerated with Buf" is rejected too and must be reworded — that
  # false positive fails CLOSED. Lowercase tool mentions ("generated by
  # protoc-gen-go") stay clean; green row in the self-test.
...
check_attribution() { # check_attribution <label> <text>
  ...
  for p in "${ATTR_PATTERNS[@]}"; do
    ...
    if [ "$rc" -eq 0 ]; then
      ...
      echo "conventions: GUARD ERROR — grep exited $rc on pattern $p"
      fail=1
    fi
  done
}
